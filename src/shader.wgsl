...
fn march(pos: Point, dir: Point) -> MarchResult {
    ...
    for (var iter = 0; iter < MARCH_MAX_ITERS; iter++) {
        ...
        for (var idx = 0; idx < MAX_BODIES; idx++) {
            ...
            if sdf < world_sdf { best_idx = idx; world_sdf = sdf; }
            if SMOOTH_MARCH { smooth_sdf += smooth_exp(sdf); }
        }
        if SMOOTH_MARCH { smooth_sdf = -SMOOTH_K * log2(smooth_sdf); }
        var dt: f32;
        if SMOOTH_MARCH { dt = smooth_sdf; } else { dt = world_sdf; }
        t += dt;
        if dt < MARCH_EPSILON { return MarchResult(t, best_idx); }
        best_idx = -2;
...
//        }
        var color = state.global_light_color;
        if primary.idx == -1 {
            color *= 0.1;
        } else {
            color *= state.bodies[primary.idx].material.ambient;
        }
...
//            color += state.point_lights[pl_idx].color * state.bodies[primary.idx].material.diffuse * max(0.0, -dot(dir, normal));
            if primary.idx == -1 {
                color += state.point_lights[pl_idx].color * 0.1;
            } else {
